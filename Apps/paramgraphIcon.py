...
def add_emoji(row):
        if row['sensor_type'] == 'Temperature':
            if row['value'] > 90:  # Example threshold
                return "🔥"  # Fire emoji (high temperature)
            elif row['value'] < 10:  # Example threshold
                return "❄️" # Snowflake Emoji (low temperature)
            else:
                return ""
        elif row['sensor_type'] == 'Pressure':
            if row['value'] > 95:  # Example threshold
                return "⚠️"  # Warning sign emoji (high pressure)
            elif row['value'] < 5:
                return "💧" # Droplet emoji (low pressure)
            else:
                return ""
        else:
            return "" # Handle other sensor types if needed
# DynamoDB Resource
try:
...
try:
    ...
    if not items:
        ...
        if not filtered_df.empty: 
            ...
            filtered_df['Alert'] = filtered_df.apply(add_emoji, axis=1)
